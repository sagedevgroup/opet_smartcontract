...
contract('Opet - tranfer cases', async(accounts)=>{
    it('[Func Test] transfer <conditions - transfer paused>', async()=>{
        ...
        try {
            let result = await token.transfer(accounts[1], 1 * (10 ** 8))
        } catch(err){
            assert.isAbove(err.message.search('revert'), -1, 'transfer paused settings not revert transfer action')
        }
    })
})
...
contract('Opet - StandardToken - transferFrom', async(accounts)=>{
    beforeEach(async()=>{
        let token = await OpetToken.new()
        this.token = token
        let mint = await token.mint(accounts[0], 1000)
        if(!mint){
            assert.isOk(false, 'mint fail')
        }
        let canTransfer = await token.unpauseTransfer()
    ...
    it('[Func Test] transferFrom - <conditions - normal account, normal amount>', async()=>{
        ...
        assert.equal(balance.toString(), '990', 'balance of from acount is not right')
    ...
    it('[Func Test] transferFrom - <conditions - from account has not enough amount, normal amount>', async()=>{
        ...
        assert.isOk(result, 'should be ok')
        try{
            result = await this.token.transferFrom(accounts[0], accounts[1], 1001, {from:accounts[1]})
            assert.isOk(false, 'should be fail')
        } catch(err){
            //console.log(err.message)
